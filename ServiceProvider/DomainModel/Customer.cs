...
namespace DomainModel
{
    public class Customer
    {
        ...
        [RegexValidator(@"(\(?\d{4}\)?-?.? *\d{3}-?.? *-?\d{3})|(^$)", ErrorMessage = "Invalid phone number!")]
        public String Phone
        {
            get;
            set;
        ...
        internal static void Validate(Customer customer, ValidationResults results)
        {
            if (true)
            {
                results.AddResult
                    (
                        new ValidationResult("some reason from SelfValidation method", customer, "ValidateMethod", "error", null)
                    );
            }
        }
